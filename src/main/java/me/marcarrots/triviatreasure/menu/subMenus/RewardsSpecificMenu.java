...
import me.marcarrots.triviatreasure.Rewards;
...
public class RewardsSpecificMenu extends Menu {
    ...
    public RewardsSpecificMenu(TriviaTreasure triviaTreasure, Player player, int place) {
    ...
    public void handleMenuClick(InventoryClickEvent event) {
        ...
        switch (event.getSlot()) {
            case 38: // change money
                ...
                conversation = conversationFactory.withFirstPrompt(new ConversationPrompt(triviaTreasure, PromptType.SET_MONEY).setPlace(place)).withLocalEcho(false).withTimeout(60).buildConversation(player);
                conversation.begin();
                player.closeInventory();
                break;
            case 42: // change experience
                event.setCancelled(true);
                conversation = conversationFactory.withFirstPrompt(new ConversationPrompt(triviaTreasure, PromptType.SET_EXPERIENCE).setPlace(place)).withLocalEcho(false).withTimeout(60).buildConversation(player);
                conversation.begin();
                player.closeInventory();
                break;
            case 43: // set fireworks
                event.setCancelled(true);
                Rewards reward = triviaTreasure.getRewards()[place];
                reward.flipSummonFireworks();
                placeFireworks(reward);
                break;
            case 44: // change win message
                event.setCancelled(true);
                conversation = conversationFactory.withFirstPrompt(new ConversationPrompt(triviaTreasure, PromptType.SET_WIN_MESSAGE).setPlace(place)).withLocalEcho(false).withTimeout(60).buildConversation(player);
                conversation.begin();
                player.closeInventory();
                break;
        }
    ...
    public void handleMenuClose(InventoryCloseEvent event) {
        ...
        triviaTreasure.getLogger().log(Level.INFO, String.format("%s has modified the rewards settings for place %d.", event.getPlayer().getName(), place));
        triviaTreasure.getRewards()[place].setItems(items);
    ...
    public void setMenuItems() {
        ...
        Rewards reward = null;
        try {
            reward = triviaTreasure.getRewards()[place];
        } catch (Exception e) {
            e.printStackTrace();
        }
