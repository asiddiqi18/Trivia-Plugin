...
public class PlayerScoreHolder {
    ...
    public void addOnlinePlayersToGame() {
        ...
        for (Player player : onlinePlayers) {
            triviaTreasure.getStats().addGameParticipated(player);
    ...
    public void deliverRewardsToWinners() {
        int displayAmount = Math.min(scores.size(), triviaTreasure.getConfig().getInt("Top winner amount", 3));
        ...
        for (int i = 0; i < displayAmount; i++) {
            ...
            if (score != null) {
                ...
                Rewards[] rewards = triviaTreasure.getRewards();
