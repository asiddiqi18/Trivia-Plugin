...
import me.marcarrots.triviatreasure.listeners.EntityDamage;
...
public final class TriviaTreasure extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        new UpdateChecker(this, 80401).getVersion(newVersion -> {
            ...
            if (Integer.parseInt(newVersion.substring(2)) > Integer.parseInt(currentVersion.substring(2))) {
                updateNotice = String.format("%s - There is a new version available for Trivia Treasure (new version: %s, current version: %s)! Get it at: %s.", ChatColor.AQUA + "[Trivia Treasure]" + ChatColor.YELLOW, ChatColor.GREEN + newVersion + ChatColor.YELLOW, ChatColor.GREEN + currentVersion + ChatColor.YELLOW, ChatColor.WHITE + "https://www.spigotmc.org/resources/trivia-easy-to-setup-game-%C2%BB-custom-rewards-%C2%BB-in-game-gui-menus-more.80401/" + ChatColor.YELLOW);
